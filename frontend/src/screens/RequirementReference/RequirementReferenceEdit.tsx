...
interface RequirementReferenceEditProps {
  record: RequirementReferenceRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const RequirementReferenceEdit: React.FC<RequirementReferenceEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<RequirementReferenceRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<RequirementReferenceRecord>) =>
      dataProvider.update('RequirementReference', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'RequirementReference', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit RequirementReference</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <LookupSelect label="Source Standard *" value={String(form.sourceStandard ?? '')} onChange={(v) => setField('sourceStandard', v ? Number(v) : null)} searchable={false} options={[{ label: 'Authority', value: '0' }, { label: 'ISO', value: '1' }, { label: 'InternalProcedure', value: '2' }]} />
                </div>
                <div>
