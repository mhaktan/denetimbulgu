...
import { FindingLevelCreate } from './FindingLevelCreate';
import { FindingLevelEdit } from './FindingLevelEdit';
...
type FindingLevelRecord = {
  id: string | number;
  code: string;
  name: string;
  defaultClosureDays: number;
  [key: string]: unknown;
};
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'defaultClosureDays', header: "Default Closure Days", sortable: true },
];
...
export const FindingLevelList: React.FC = () => {
  const list = useListQuery<FindingLevelRecord>({ resource: 'FindingLevel' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<FindingLevelRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<FindingLevelRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('FindingLevel', (ids) => { ids.forEach(id => triggerFlows('delete', 'FindingLevel', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as FindingLevelRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create FindingLevel" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <FindingLevelCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <FindingLevelEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
