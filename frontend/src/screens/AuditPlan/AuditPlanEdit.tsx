...
interface AuditPlanEditProps {
  record: AuditPlanRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const AuditPlanEdit: React.FC<AuditPlanEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<AuditPlanRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<AuditPlanRecord>) =>
      dataProvider.update('AuditPlan', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'AuditPlan', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit AuditPlan</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <TkInput mode="text" label="Period *" value={String(form.period ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('period', v))(e.detail)} />
                </div>
                <div>
                  <TkInput mode="text" label="Scope Description *" value={String(form.scopeDescription ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('scopeDescription', v))(e.detail)} />
                </div>
                <div>
                  <LookupSelect label="Status *" value={String(form.status ?? '')} onChange={(v) => setField('status', v ? Number(v) : null)} searchable={false} options={[{ label: 'Draft', value: '0' }, { label: 'Approved', value: '1' }]} />
                </div>
                <div>
                  <TkInput mode="number" label="Quality Manager Approver Id" value={String(form.qualityManagerApproverId ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('qualityManagerApproverId', Number(v)))(e.detail)} />
                </div>
            </div>
          </div>
