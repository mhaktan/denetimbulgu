...
import { ActionProgressNoteCreate } from './ActionProgressNoteCreate';
import { ActionProgressNoteEdit } from './ActionProgressNoteEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'note', header: "Note", sortable: true, searchable: true, filterType: 'text' },
...
// ---------------------------------------------------------------------------
// ActionProgressNoteList
// ---------------------------------------------------------------------------

export const ActionProgressNoteList: React.FC = () => {
  const list = useListQuery<ActionProgressNoteRecord>({ resource: 'ActionProgressNote' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<ActionProgressNoteRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<ActionProgressNoteRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('ActionProgressNote', (ids) => { ids.forEach(id => triggerFlows('delete', 'ActionProgressNote', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as ActionProgressNoteRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create ActionProgressNote" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <ActionProgressNoteCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <ActionProgressNoteEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
