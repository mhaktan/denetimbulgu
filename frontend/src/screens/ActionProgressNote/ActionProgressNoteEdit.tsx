...
interface ActionProgressNoteEditProps {
  record: ActionProgressNoteRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const ActionProgressNoteEdit: React.FC<ActionProgressNoteEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<ActionProgressNoteRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<ActionProgressNoteRecord>) =>
      dataProvider.update('ActionProgressNote', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'ActionProgressNote', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit ActionProgressNote</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <TkInput mode="text" label="Note *" value={String(form.note ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('note', v))(e.detail)} />
                </div>
                <div>
                  <TkDatepicker label="Note Date *" value={String(form.noteDate ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('noteDate', v))(e.detail)} />
                </div>
                <div>
                  <LookupSelect label="Düzeltici Aksiyon *" resource="CorrectiveAction" value={String(form.correctiveActionId ?? '')} onChange={(v) => setField('correctiveActionId', v)} displayField="actionDescription" />
                </div>
                <div>
