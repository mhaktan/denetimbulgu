...
interface AuditTypeEditProps {
  record: AuditTypeRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const AuditTypeEdit: React.FC<AuditTypeEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<AuditTypeRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<AuditTypeRecord>) =>
      dataProvider.update('AuditType', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'AuditType', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit AuditType</h2>
