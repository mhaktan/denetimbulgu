...
import { AuditTypeCreate } from './AuditTypeCreate';
...
export const AuditTypeList: React.FC = () => {
  const list = useListQuery<AuditTypeRecord>({ resource: 'AuditType' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<AuditTypeRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<AuditTypeRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('AuditType', (ids) => { ids.forEach(id => triggerFlows('delete', 'AuditType', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as AuditTypeRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create AuditType" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <AuditTypeCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <AuditTypeEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
