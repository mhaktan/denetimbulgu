...
using DenetimBulgu.Authentication;
using DenetimBulgu.Entities;
using DenetimBulgu.EntityFrameworkCore;

namespace DenetimBulgu.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = DenetimBulgu.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else
